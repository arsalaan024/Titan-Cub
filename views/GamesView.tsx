...
const GamesView: React.FC<GamesViewProps> = ({ user, onSync }) => {
  ...
  useEffect(() => { setLeaderboard(db.getLeaderboard()); }, [activeGame]);

  const saveScore = useCallback((points: number, accuracy: number, levelCleared: boolean, isGauntlet: boolean = false) => {
    if (user) { db.updateUserGameStats(user.id, points, accuracy, levelCleared, isGauntlet); onSync(); }
  }, [user, onSync]);
